...
class NewClientFormReflex(Reflex):
    def paginate(self, page):
        paginator = Paginator(
            get_client_list(**parse_form_fields("client", self.params)), 10
        )
    ...
    def update(self, field_d):
        print(
            f"updating {field_d['form']} with {field_d['field']}:{field_d['value']}"
        )
        pk = self.session.get("pk")
        if pk is None:
            raise TypeError("pk is not set")
    ...
    def refill_forms(self):
        ...
        if pk is not None:
            (
                self.client_form,
                self.client_profile_form,
                self.client_family_formset,
            ) = _refill_forms_from_db(pk)
        else:
            (
                self.client_form,
                self.client_profile_form,
            ) = _refill_forms_from_params(self.params)
...
def _refill_forms_from_db(pk):
    ...
    try:
        ...
        client_family_formset = ClientFamilyFormSet(
            prefix="family", initial=family.values()
        )
    except ClientProfile.DoesNotExist:
        client_profile_form = ClientProfileForm()
...
def _refill_forms_from_params(params):
    ...
    client_profile_form = ClientProfileForm(
        initial={
            ...
            "dietary_considerations": client_profile_fields.get(
                "dietary_considerations"
            ),
            "date_of_first_visit": client_profile_fields.get(
                "date_of_first_visit"
            ),
            "date_of_last_visit": client_profile_fields.get(
                "date_of_last_visit"
            ),
